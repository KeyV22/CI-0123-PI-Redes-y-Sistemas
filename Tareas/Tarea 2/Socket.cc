...
size_t Socket::recvFrom(void * buffer, size_t size, void * addr){
   socklen_t addrLen;
   if(this->IPv6){
      addrLen=sizeof(struct sockaddr_in6); //scktlen es para tama;os de direcciones de socket
                                          //Tama;o de la estructure IPv6
   }else{
      addrLen=sizeof(struct sockaddr_in); //scktlen es para tama;os de direcciones de socket

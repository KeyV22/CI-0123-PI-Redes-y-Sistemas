...
/**
  ...
  *
 **/
std::string HacerPeticion( const char * host, const char * puerto, const std::string & path, Logger & log, bool ipv6 = false ) {

   Socket * conexion = new Socket( 's', ipv6 );
...
int main( int argc, char ** argv ) {
   ...
   const char * host = ipv6 ? "::1" : "127.0.0.1";
   if ( ipv6 && argc > 2 ) host = argv[2];         // ej: ./Cliente.out 6 fe80::xxxx%interfaz
   ...
   while ( corriendo ) {
      ...
      if ( "1" == comando ) {
         ...
         std::string resp = HacerPeticion( host, puerto, "/categorias", log, ipv6 );
         ...
         std::string resp = HacerPeticion( host, puerto, "/productos/" + UrlEncode( categoria ), log, ipv6 );
         ...
         std::string resp = HacerPeticion( host, puerto, "/agregar/" + idStr + "/" + cantStr, log, ipv6 );
         ...
         std::string resp = HacerPeticion( host, puerto, "/factura?items=" + items.str(), log, ipv6 );

...
bool FileSystem::actualizarStock(uint32_t idProducto, int32_t delta, std::string* motivoError) {
    // Mismo recorrido que reservarProducto, pero permite sumar stock
    // (reabastecer) ademas de restar (correccion de inventario, sin pasar
    // de la orden de un cliente).
    uint32_t actualIndice = superbloque.punteroIndiceCategorias;
    char bufferIndice[TAMANO_BLOQUE];
    char bufferBloque[TAMANO_BLOQUE];
    ...
    while (actualIndice != FIN_CADENA) {
        ...
        for (auto& entrada : indice.entradas) {
            ...
            while (actualBloque != FIN_CADENA) {
                ...
                for (auto& p : bloque.productos) {
                    if (p.idProducto == idProducto) {
                        long long nuevoStock = (long long)p.cantidadDisponible + delta;
                        if (nuevoStock < 0) {
                            if (motivoError) *motivoError = "STOCK_INSUFICIENTE";
                            return false;
                        }
                        p.cantidadDisponible = (uint32_t)nuevoStock;
                        bloque.serializar(bufferBloque);
                        escribirBloque(actualBloque, bufferBloque);
                        return true;
                    }
...
bool FileSystem::eliminarProducto(uint32_t idProducto, std::string* motivoError) {
    // Recorre igual que reservarProducto, pero llevando el bloque ANTERIOR de
    // la cadena de categoria, para poder desenganchar y liberar el bloque
    // actual si queda vacio (y no es el primero de la cadena: ese se deja
    // vivo, aunque quede vacio, para que la entrada del indice de categorias
    // siempre siga apuntando a un bloque existente).
    uint32_t actualIndice = superbloque.punteroIndiceCategorias;
    char bufferIndice[TAMANO_BLOQUE];
    char bufferBloque[TAMANO_BLOQUE];
    char bufferPrev[TAMANO_BLOQUE];
    ...
    while (actualIndice != FIN_CADENA) {
        ...
        for (auto& entrada : indice.entradas) {
            uint32_t bloquePrevio = FIN_CADENA;
            uint32_t actualBloque = entrada.bloqueInicio;
            ...
            while (actualBloque != FIN_CADENA) {
                ...
                for (size_t i = 0; i < bloque.productos.size(); i++) {
                    if (bloque.productos[i].idProducto != idProducto) continue;

                    bloque.productos.erase(bloque.productos.begin() + i);

                    if (!bloque.productos.empty() || bloquePrevio == FIN_CADENA) {
                        // sigue teniendo productos, o es el primer bloque de
                        // la categoria: se reescribe tal cual (puede quedar
                        // vacio si es el primero, y esta bien)
                        bloque.serializar(bufferBloque);
                        escribirBloque(actualBloque, bufferBloque);
                    } else {
                        // bloque encadenado (no el primero) que quedo vacio:
                        // se desengancha de la cadena y se libera de verdad
                        leerBloque(bloquePrevio, bufferPrev);
                        BloqueCategoria anterior;
                        anterior.deserializar(bufferPrev);
                        anterior.siguienteBloque = bloque.siguienteBloque;
                        anterior.serializar(bufferPrev);
                        escribirBloque(bloquePrevio, bufferPrev);
                        liberarBloque(actualBloque);
                    }

                    return true;
                }

                bloquePrevio = actualBloque;
                actualBloque = bloque.siguienteBloque;
            }
        }
        actualIndice = indice.siguienteBloque;
